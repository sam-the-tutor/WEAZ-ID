...
export default function dashboard() {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      setIsLoading(true)
      await weazIDActor
        .setMyWallet(values.walletName, Principal.fromText(values.principalID))
        .then((results) => {
          setChange(results)
          setIsLoading(false)
        })
    ...
    >
      ...
      >
        <Box
          ...
          xs={12}
        >
          <Grid item xs={12} md={5} margin={3} d>
            <Card>
              <Card>
                <CardContent>
                  <form onSubmit={formik.handleSubmit}>
                    ...
                    {formik.errors.walletName && formik.touched.walletName ? (
                      <div>{formik.errors.walletName}</div>
                    ) : null}
                    <input
                      type='text'
                      id='principalID'
                    ...
                    {formik.errors.principalID && formik.touched.principalID ? (
                      <div>{formik.errors.principalID}</div>
                    ) : null}
                    <Button
                      type='submit'
                      sx={{
          ...
          <Grid item xs={12} md={12} borderRadius='10px'>
            <Card>
              <Typography
                variant='h5'
                component='h5'
